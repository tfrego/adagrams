...
describe 'Adagrams' do
  ...
  describe 'score_word method' do
    ...
    it 'adds an extra 8 points if the word is 7 or more characters long' do
      score_word("XXXXXXX").must_equal 64
      score_word("XXXXXXXX").must_equal 72
      score_word("XXXXXXXXX").must_equal 80
    end
  end
  ...
  describe 'highest_score_from method' do
    ...
    it 'in case of tied score, prefers the word with fewer letters' do
      ...
      score_word(words.first).must_equal 12
      score_word(words.last).must_equal 12

      best_word = highest_score_from words

      best_word[:word].must_equal 'WWW'
      best_word[:score].must_equal 12
    end
    ...
    it 'in case of tied score, prefers most the word with 10 letters' do
      ...
      score_word(words.first).must_equal 18
      score_word(words.last).must_equal 18

      best_word = highest_score_from words

      best_word[:word].must_equal 'AAAAAAAAAA'
      best_word[:score].must_equal 18
    end
    ...
    it 'in case of tied score and same length words, prefers the first word' do
      ...
      best_word[:word].must_equal words.first
      best_word[:score].must_equal 18
    end
